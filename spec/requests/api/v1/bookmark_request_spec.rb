...
describe 'Bookmark API' do
  it 'can create a bookmark' do
    ...
    # bookmarks do not have validations other than minutes, so it's ok to leave a space blank (page number is not included)
  ...
  it 'throws an error if a bookmark cannot be created' do
    ...
    expect(error[:error]).to eq('must include minutes')
  end
end
